      ...
      <v-row>
        <v-col>
          <div class="check-header blue1 text-center">
            Check Out {{ formatDate(reservation.date) }}/{{
        ...
        <v-col md="6">
          ...
          <div class="btn-action-upload">
            ...
            <v-btn
              class="btn-green1"
              @click="makeCheckIn()"
              title="Haz clic para hacer Check Out"
            >
              <img class="mr-1" src="/check/checked.png" />Check Out
            </v-btn>
          </div>
        </v-col>
      </v-row>
  ...
  methods: {
    ...
    async makeCheckIn() {
      ...
      if (
        ...
        let res = await this.$store.dispatch("reservations/makeCheckOut", {
